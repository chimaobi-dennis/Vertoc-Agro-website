...
try {
  ...
  if (has005) {
    ...
    if (inbound) {
      ...
      await step('GET /messages/threads groups the conversation', async () => { const t = await api('/messages/threads'); const th = t.find(x => x.client_id === client2.id); if (!th || th.unread < 1 || !th.last || th.key !== `c${client2.id}`) throw new Error(JSON.stringify(t).slice(0, 160)); const team = (S.email.notify_to || S.email.reply_to || '').toLowerCase(); if (team && t.some(x => x.email === team)) throw new Error('team notifications leaked into conversations'); return `${t.length} threads · ${th.name} · ${th.unread} unread · no team mail ✓` })
      await step('GET /messages/thread folds the quoted history', async () => { const rows = await api(`/messages/thread?key=c${client2.id}`); const m = rows.find(x => x.id === inbound.id); if (!m || m.text !== 'Thanks, noted.' || !/wrote:/.test(m.quoted) || m.html !== undefined) throw new Error(JSON.stringify({ text: m?.text, quoted: m?.quoted }).slice(0, 160)); if (rows.some(x => /^New email from/.test(x.subject))) throw new Error('the team notification is inside the client thread'); return `${rows.length} messages · text "${m.text}" · quoted folded ✓` })
      await step('POST /messages reply_to_id threads by headers (dry run)', async () => { const r = await api('/messages', { method: 'POST', body: { reply_to_id: inbound.id, body: 'e2e reply in thread' } }); if (r.in_reply_to !== `<${emailId}@e2e>` || !/^Re: /.test(r.subject) || r.to_email !== 'buyer3@e2e.invalid' || r.client_id !== client2.id || !r.provider_message_id) throw new Error(JSON.stringify(r).slice(0, 200)); if (r.html.includes('wrote:')) throw new Error('old text was pasted into the reply'); return `${r.subject} · In-Reply-To ${r.in_reply_to}` })
      ...
      await step('POST /messages/thread/read leaves nothing unread', async () => { const r = await api('/messages/thread/read', { method: 'POST', body: { key: `c${client2.id}` } }); const t = await api('/messages/threads'); const th = t.find(x => x.client_id === client2.id); if (!th || th.unread !== 0) throw new Error('unread ' + th?.unread); return `${r.read} marked now, ${th.unread} unread` })
