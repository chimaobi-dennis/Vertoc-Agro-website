...
try {
  ...
  if (has005) {
    ...
    if (inbound) {
      ...
      await step('a custom label joins the list; clear + remove', async () => { const r = await api('/messages/thread/label', { method: 'PUT', body: { key: th.key, label: 'e2e custom label' } }); if (r.label?.name !== 'e2e custom label') throw new Error(JSON.stringify(r)); const c = await api('/messages/labels'); if (!c.labels.some(l => l.name === 'e2e custom label')) throw new Error('not in catalogue'); const off = await api('/messages/thread/label', { method: 'PUT', body: { key: th.key, label: null } }); if (off.label !== null) throw new Error('not cleared'); const l2 = await api('/messages/labels', { method: 'PUT', body: { labels: c.labels.filter(l => l.name !== 'e2e custom label') } }); if (l2.labels.some(l => l.name === 'e2e custom label')) throw new Error('not removed'); return 'custom → listed → cleared → removed ✓' })
