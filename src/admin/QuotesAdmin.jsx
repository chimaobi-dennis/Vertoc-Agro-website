      ...
      <PageHeader eyebrow="Sales" title="Quotes" description={rows ? `${visible.length} ${status === 'all' ? '' : status} quote${visible.length === 1 ? '' : 's'}` : ' '}
        action={
          <div className="flex flex-wrap gap-2">
            <Link to="/admin/quotes/fields"><Button variant="outline"><Settings2 className="w-4 h-4" />Fields</Button></Link>
            <Link to="/admin/quotes/new"><Button variant="accent"><Plus className="w-4 h-4" />New quote</Button></Link>
          </div>
        } />
        ...
        <Table head={['Number', 'Client', 'Title', 'Total', 'Status', 'Valid until', '']}>
          ...
          {rows && visible.map(r => (
            <tr key={r.id} className="hover:bg-muted/40">
              <Td><Link to={`/admin/quotes/${r.id}`} className="font-medium hover:text-accent">{r.number}</Link></Td>
              <Td>{r.client_id ? <Link to={`/admin/clients/${r.client_id}?tab=quotes`} className="hover:text-accent">{r.client_name || '—'}</Link> : (r.client_name || '—')}</Td>
              ...
              <Td className="text-right"><Link to={`/admin/quotes/${r.id}`} className="text-xs font-semibold text-accent">Open →</Link></Td>
            </tr>
          ))}
